...
public class EmojiCompactPrefixTree<TValue> : IReadOnlyDictionary<string, TValue>, IReadOnlyList<KeyValuePair<string, TValue>>
{
    /// <summary>
    ...
    public int TreeCapacity
    {
        ...
        {
            if (value < TreeSize)
                MDThrowHelper.ThrowArgumentOutOfRangeException(ExceptionArgument.value, ExceptionReason.SmallCapacity);
    ...
    /// <summary>
    /// Gets the number of key/value pairs contained in the <see cref="EmojiCompactPrefixTree{TValue}"/>
    /// </summary>
    ...
    /// <summary>
    /// Constructs a new <see cref="EmojiCompactPrefixTree{TValue}"/> with no initial prefixes
    /// </summary>
    public EmojiCompactPrefixTree(int matchCapacity = 0, int treeCapacity = 0, int childrenCapacity = 0)
    {
        Init(matchCapacity, treeCapacity, childrenCapacity);
    }
    /// <summary>
    /// Constructs a new <see cref="EmojiCompactPrefixTree{TValue}"/> with the supplied matches
    /// </summary>
    /// <param name="input">Matches to initialize the <see cref="EmojiCompactPrefixTree{TValue}"/> with. For best lookup performance, this collection should be sorted.</param>
    public EmojiCompactPrefixTree(ICollection<KeyValuePair<string, TValue>> input)
    {
        if (input is null) MDThrowHelper.ThrowArgumentNullException(ExceptionArgument.input);
    ...
    public KeyValuePair<string, TValue> this[int index]
    {
        ...
        get
        {
            if ((uint)index >= (uint)Count) MDThrowHelper.ThrowIndexOutOfRangeException();
    ...
    /// <summary>
    /// Adds the specified key/value pair to the <see cref="EmojiCompactPrefixTree{TValue}"/>
    /// </summary>
    ...
    /// <summary>
    /// Tries to add the key/value pair to the <see cref="EmojiCompactPrefixTree{TValue}"/> if the key is not yet present
    /// </summary>
    /// <param name="key">The key of the element to add</param>
    /// <param name="value">The value of the element to add</param>
    ...
    {
        string key = pair.Key;
        if (key is null) MDThrowHelper.ThrowArgumentNullException(ExceptionArgument.key);
        if (key.Length == 0) MDThrowHelper.ThrowArgumentOutOfRangeException(ExceptionArgument.key, ExceptionReason.String_Empty);
        ...
        HandleDuplicateKey:;
            ...
            MDThrowHelper.ThrowArgumentException(ExceptionArgument.key, ExceptionReason.DuplicateKey);
    ...
    /// <summary>
    /// Tries to find the longest prefix of text, that is contained in this <see cref="EmojiCompactPrefixTree{TValue}"/>
    /// </summary>
    ...
    public bool TryMatchLongest(ReadOnlySpan<char> text, out KeyValuePair<string, TValue> match)
    {
        ...
        for (int i = 1; i < text.Length; i++)
        {
            ...
            // This is an optimization because this cloned version of CompactPrefixTree is only being used for emoji parsing.
            // Emojis cannot include spaces, so we can skip the entire tree if we encounter a space
            if (c == ' ')
            {
                return false;
            }
    ...
    /// <summary>
    /// Tries to find a prefix of text, that is contained in this <see cref="EmojiCompactPrefixTree{TValue}"/> and is exactly text.Length characters long
    /// </summary>
    /// <param name="text">The text in which to search for the prefix</param>
    /// <param name="match">The found prefix and the corresponding value</param>
    /// <returns>True if a match was found, false otherwise</returns>
    ...
    /// <summary>
    /// Tries to find the shortest prefix of text, that is contained in this <see cref="EmojiCompactPrefixTree{TValue}"/>
    /// </summary>
    /// <param name="text">The text in which to search for the prefix</param>
    /// <param name="match">The found prefix and the corresponding value</param>
    /// <returns>True if a match was found, false otherwise</returns>
    ...
    /// <summary>
    /// Determines whether the <see cref="EmojiCompactPrefixTree{TValue}"/> contains the specified key
    /// </summary>
    /// <param name="key">The key to locate in this <see cref="EmojiCompactPrefixTree{TValue}"/></param>
    ...
    /// <summary>
    /// Gets a collection containing the keys in this <see cref="EmojiCompactPrefixTree{TValue}"/>
    /// </summary>
    ...
    /// <summary>
    /// Gets a collection containing the values in this <see cref="EmojiCompactPrefixTree{TValue}"/>
    /// </summary>
    ...
    /// <summary>
    /// Returns an Enumerator that iterates through the <see cref="EmojiCompactPrefixTree{TValue}"/>.
    ...
    /// <summary>
    /// Enumerates the elements of a <see cref="EmojiCompactPrefixTree{TValue}"/>
    /// </summary>
