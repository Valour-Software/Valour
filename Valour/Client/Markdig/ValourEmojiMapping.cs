...
/// </summary>
public class ValourEmojiMapping
{
    /// <summary>
    /// The default emoji shortcodes and smileys mapping.
    /// </summary>
    public static readonly ValourEmojiMapping DefaultEmojisAndSmileysMapping = new ValourEmojiMapping();

    /// <summary>
    /// The default emoji shortcodes mapping, without smileys.
    /// </summary>
    public static readonly ValourEmojiMapping DefaultEmojisOnlyMapping = new ValourEmojiMapping(enableSmileys: false);

    public EmojiCompactPrefixTree<string> PrefixTree { get; }
    ...
    /// </summary>
    public ValourEmojiMapping(bool enableSmileys = true)
    ...
    /// </summary>
    public ValourEmojiMapping(IDictionary<string, string> shortcodeToUnicode, IDictionary<string, string> smileyToShortcode)
    {
        if (shortcodeToUnicode is null)
            MDThrowHelper.ArgumentNullException(nameof(shortcodeToUnicode));

        if (smileyToShortcode is null)
            MDThrowHelper.ArgumentNullException(nameof(smileyToShortcode));
        ...
        PrefixTree = new EmojiCompactPrefixTree<string>(jointCount, jointCount * 2, jointCount * 2);
        ...
        foreach (var smiley in smileyToShortcode)
        {
            if (string.IsNullOrEmpty(smiley.Key) || string.IsNullOrEmpty(smiley.Value))
                MDThrowHelper.ArgumentException("The dictionaries cannot contain null or empty keys/values", nameof(smileyToShortcode));
            ...
            if (!PrefixTree.TryAdd(smiley.Key, unicode))
                MDThrowHelper.ArgumentException(string.Format("Smiley {0} is already present in the emoji mapping", smiley.Key));
        }
