...
export type TilesetPackPlanRegion = {
    id: string;
    sourceX: number;
    sourceY: number;
    width: number;
...
export type TilesetPackPlan = {
    ...
    atlasHeight: number;
    paddingTiles: number;
    regions: TilesetPackPlanRegion[];
};

export type TilesetPackageResult = {
    manifest: JsonRecord;
    plan: TilesetPackPlan;
};
...
function value<T>(source: JsonRecord, lower: string, upper: string, fallback: T): T {
    return (source[lower] ?? source[upper] ?? fallback) as T;
}
...
function nonNegativeInteger(raw: unknown, fallback = 0): number {
    const parsed = Number(raw);
...
function setCoordinate(target: JsonRecord, lower: string, upper: string, coordinate: number): void {
...
function getDefinitionSourceRect(definition: JsonRecord, tileSize: number): Omit<PendingRegion, "definitionKeys" | "wallSetKeys"> {
...
function getWallSetSourceRect(wallSet: JsonRecord, defaultTileSize: number): Omit<PendingRegion, "definitionKeys" | "wallSetKeys"> {
    const tileSize = positiveInteger(value(wallSet, "tileSize", "TileSize", defaultTileSize), defaultTileSize);
    const sourceX = nonNegativeInteger(value(
        wallSet,
        "sourceOriginX",
        "SourceOriginX",
        value(wallSet, "originX", "OriginX", 0)));
    const sourceY = nonNegativeInteger(value(
        wallSet,
        "sourceOriginY",
        "SourceOriginY",
        value(wallSet, "originY", "OriginY", 0)));
...
function validateSourceRect(region: Omit<PendingRegion, "definitionKeys" | "wallSetKeys">, sourceWidth: number, sourceHeight: number, label: string): void {
...
function validateSourceSheetContainment(
    region: Omit<PendingRegion, "definitionKeys" | "wallSetKeys">,
    ...
    const containingSheet = sheets.find((sheet: JsonRecord) => {
 ...
 */
export function createTilesetPackPlan(manifest: JsonRecord, sourceWidth: number, sourceHeight: number, paddingTiles = 1): TilesetPackPlan {
    ...
    const deduplicated = new Map<string, PendingRegion>();
    ...
    const pending = [...deduplicated.values()].sort((left, right) =>
    ...
    const totalArea = pending.reduce((sum, region) =>
        sum + (region.width + padding * 2) * (region.height + padding * 2), 0);
    ...
    let shelfHeight = 0;
    let usedWidth = 0;
    const regions: TilesetPackPlanRegion[] = [];
...
export function compileTilesetManifest(
    manifest: JsonRecord,
    plan: TilesetPackPlan,
    image: string,
    ...
    compiled.atlas = {
        ...
        packed: true
    } satisfies TilesetAtlas;

    const definitionsByKey = new Map<string, JsonRecord>();
    ...
    const wallSetsByKey = new Map<string, JsonRecord>();
...
export function isCanonicalTilesetManifest(manifest: JsonRecord): boolean {
    return manifest?.format === TILESET_FORMAT && manifest?.version === TILESET_FORMAT_VERSION;
}

export function getReconstructionRegions(manifest: JsonRecord): TilesetPackPlanRegion[] {
    ...
    const regions = new Map<string, TilesetPackPlanRegion>();
...
function drawExtrudedRegion(
    context: CanvasRenderingContext2D,
    image: CanvasImageSource,
    region: TilesetPackPlanRegion,
    paddingPixels: number): void {
    context.drawImage(
        image,
        region.sourceX,
        region.sourceY,
    ...
    context.drawImage(image, region.sourceX, region.sourceY, region.width, 1,
        left, top - 1, region.width, 1);
    context.drawImage(image, region.sourceX, region.sourceY + region.height - 1, region.width, 1,
        left, bottom, region.width, 1);
    context.drawImage(image, region.sourceX, region.sourceY, 1, region.height,
        left - 1, top, 1, region.height);
    context.drawImage(image, region.sourceX + region.width - 1, region.sourceY, 1, region.height,
        right, top, 1, region.height);
...
export async function renderTilesetPackage(
    sourceImage: CanvasImageSource & { width: number; height: number },
    manifest: JsonRecord,
    imageFileName: string,
    sourceSha256: string): Promise<{ manifest: JsonRecord; json: string; imageBlob: Blob; plan: TilesetPackPlan }> {
...
export async function reconstructTilesetSource(
    atlasImage: CanvasImageSource,
    manifest: JsonRecord): Promise<{ imageBlob: Blob; width: number; height: number }> {
    if (!isCanonicalTilesetManifest(manifest)) {
        throw new Error("Unsupported tileset manifest format.");
    }
