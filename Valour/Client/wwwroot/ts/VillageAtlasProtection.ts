...
export function isProtectedVillageAtlas(url: string): boolean {
    return String(url).split(/[?#]/, 1)[0].endsWith(".vtex.bin");
}

export function decodeVillageAtlas(encoded: Uint8Array): Uint8Array<ArrayBuffer> {
...
export function resolveVillageImageUrl(url: string): Promise<string> {
    if (!isProtectedVillageAtlas(url)) return Promise.resolve(url);
    ...
    const pending = (async () => {
        ...
        if (!response.ok) throw new Error(`Unable to load Village atlas (${response.status}).`);
...
export async function resolveVillagePreviewImages(urls: string[]): Promise<Record<string, string>> {
    return Object.fromEntries(await Promise.all([...new Set(urls.filter(Boolean))].map(async url =>
        [url, await resolveVillageImageUrl(url)])));
}
