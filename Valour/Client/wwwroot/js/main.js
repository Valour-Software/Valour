...
// Web Share API wrapper. Returns true if the platform handled the share
// (including the user dismissing the sheet), false if unsupported.
window.valourShare = async function (title, text, url) {
    if (!navigator.share) {
        return false;
    }

    // Only include populated fields - extra text alongside a url gets
    // concatenated by many share targets and breaks pasted links
    const data = {};
    ...
    try {
        ...
        // AbortError means the user closed the share sheet - still handled
        return e && e.name === 'AbortError';
    }
};

window.valourGpuAcceleration = {
    setEnabled: function (enabled) {
        document.documentElement.classList.toggle('force-gpu-acceleration', !!enabled);
    }
};
...
window.blazorFuncs = {
    registerClient: function (caller) {
        ...
        updateAvailablePromise
            .then(isAvailable => {
                if (isAvailable) {
                    caller.invokeMethodAsync("OnServiceUpdateAvailable");
                }
                else {
                    caller.invokeMethodAsync("OnServiceUpdateUnavailable");
                }
            })
            .catch(error => {
