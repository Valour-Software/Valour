...
import type {
    ...
    TileDefinition,
    TextureCache
} from "../../../wwwroot/ts/VillageTileRendering.js";
...
type EditorState = {
    ...
    mapHeight: number;
    tiles: string[];
    tileBrushStrengths: number[];
    tileBrushWeights: number[];
    tileBrushKeys: string[];
    tileBrushStrokeIds: number[];
    terrainGrid: string[];
    terrainIndex: TerrainIndex;
    terrainDirty: boolean;
    sprites: MapSprite[];
    nextSpriteId: number;
    nextBrushStrokeId: number;
    currentBrushStrokeId: number;
    definitions: TileDefinition[];
    definitionsByKey: Map<string, TileDefinition>;
    brushes: BrushDefinition[];
    brushesByKey: Map<string, BrushDefinition>;
    selectedTileKey: string;
    selectedBrushKey: string;
    selectedSpriteKey: string;
    selectedTerrainKey: string;
    ...
    lastPaintTile: { x: number; y: number } | null;
...
type MapEditorRuntime = {
    setTool: (tool: string) => void;
    setSelectedTile: (key: string) => void;
    setSelectedBrush: (key: string) => void;
    setSelectedSprite: (key: string) => void;
    setSelectedTerrain: (key: string) => void;
...
export async function init(canvasId: string, dotNetRef: DotnetObject, config: EditorConfig): Promise<MapEditorRuntime> {
    ...
    state.onKeyDown = (event) => {
        ...
        const step = 32;
        if (key === "w" || key === "arrowup") state.offsetY += step;
        else if (key === "s" || key === "arrowdown") state.offsetY -= step;
        else if (key === "a" || key === "arrowleft") state.offsetX += step;
        else if (key === "d" || key === "arrowright") state.offsetX -= step;
        else return;
...
function resizeCanvas(state: EditorState): void {
    const rect = state.canvas.getBoundingClientRect();
 ...
 */
function ensureCanvasSize(state: EditorState): void {
    const rect = state.canvas.getBoundingClientRect();
...
function fitMap(state: EditorState): void {
...
function resizeMap(state: EditorState, width: number, height: number): void {
...
function applyToolAt(state: EditorState, x: number, y: number): void {
...
function paintTerrainRadiusAt(state: EditorState, x: number, y: number, terrainKey: string): void {
 ...
 */
function resolveTerrainTiles(state: EditorState): void {
    const resolved = state.rendering.resolveTerrainGrid(
        state.terrainGrid,
        state.mapWidth,
        state.mapHeight,
        state.terrainIndex);
...
function paintTileRadiusAt(state: EditorState, x: number, y: number, tileKey: string): void {
    forEachTileInRadius(state, x, y, state.tileRadius, (tileX, tileY) => {
        paintTileAt(state, tileX, tileY, tileKey);
    });
}

function paintTileAt(state: EditorState, x: number, y: number, tileKey: string): void {
 ...
 */
function clearTerrainAt(state: EditorState, index: number): void {
    if (!state.terrainGrid[index]) {
        return;
    }
...
function paintBrushAt(state: EditorState, x: number, y: number): void {
    ...
    for (let cellY = 0; cellY < brush.size; cellY++) {
        for (let cellX = 0; cellX < brush.size; cellX++) {
            ...
            paintBrushCellAt(
                ...
                brush.key,
                cell.tileKey,
                getBrushCellPriority(cellX, cellY, brush.size, cell.strength),
                cell.weight);
        }
    }
}

function getBrushCellPriority(cellX: number, cellY: number, brushSize: number, strength: number): number {
...
function paintBrushCellAt(state: EditorState, x: number, y: number, brushKey: string, tileKey: string, priority: number, weight: number): void {
    if (!tileKey || !isInsideMap(state, x, y)) {
        return;
    }
...
function isLowerBrushPriority(priority: number, weight: number, currentPriority: number, currentWeight: number): boolean {
...
function placeSpriteAt(state: EditorState, x: number, y: number): void {
...
function eraseAt(state: EditorState, x: number, y: number): void {
...
function findTopSpriteIndexAt(state: EditorState, x: number, y: number): number {
...
function drawTileLayer(state: EditorState, image: HTMLImageElement, cellSize: number): void {
    const { ctx } = state;
...
function drawSprites(state: EditorState, image: HTMLImageElement, cellSize: number): void {
    const { ctx } = state;
...
function drawGrid(state: EditorState, cellSize: number): void {
    const { ctx } = state;
...
function drawHover(state: EditorState, cellSize: number): void {
    if (!state.hoverTile) {
        return;
    }
    ...
    let width = 1;
    let height = 1;
    let brush: BrushDefinition | undefined;
    let definition: TileDefinition | undefined;
    if (state.tool === "Brush") {
...
function getMapPoint(state: EditorState, event: MouseEvent): { x: number; y: number } | null {
...
function getWorldPoint(state: EditorState, event: MouseEvent | WheelEvent): { x: number; y: number } {
    const rect = state.canvas.getBoundingClientRect();
    const cellSize = getCellSize(state);
    return {
...
function getCellSize(state: EditorState): number {
    return state.tileSize * state.scale;
}

function isInsideMap(state: EditorState, x: number, y: number): boolean {
    return x >= 0 && y >= 0 && x < state.mapWidth && y < state.mapHeight;
}

function notifyMapChanged(state: EditorState): void {
...
function exportMap(state: EditorState) {
    return {
...
function countPaintedTiles(state: EditorState): number {
