...
public class MessageService
{
    ...
    public async Task<TaskResult<Message>> PostMessageAsync(Message message, MessageWriteOptions writeOptions = null)
    {
        ...
        // Mention notifications carry the message content, so they are only
        // sent once automod has allowed the message and it has been accepted.
    ...
    public async Task<TaskResult<Message>> EditMessageAsync(Message updated)
    {
        ...
        if (updated.PlanetId is not null && updated.Content != old.Content)
        {
            ...
            try
            {
                scanResult = await _automodService.ScanMessageAsync(updated, memberModel, isEdit: true);
                if (!scanResult.AllowMessage)
                {
                    if (scanResult.ActionsToRun.Count > 0 && memberModel is not null)
                        await _automodService.RunMessageActionsAsync(scanResult.ActionsToRun, memberModel, updated);

                    return TaskResult<Message>.FromFailure("Message edit blocked by automod.");
                }
            }
            catch (Exception e)
            {
                _logger.LogError(e, "Failed to scan edited message with automod");
                return TaskResult<Message>.FromFailure("Automod scan failed. Message was not edited.");
            }
        }
